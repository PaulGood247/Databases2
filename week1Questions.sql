...
select * from cd_attendee where coder_phone is null;
SELECT  coder_name , count(*) FROM cd_attends join cd_attendee using (coder_id) group by coder_name HAVING COUNT(*)>2;
Select organizer_name, cd_location.loc_name from cd_organizer join cd_coding_session using (organizer_id) join cd_location using(loc_id); 
